...
// является ли второе число кратным первому. (Кратно = первое число делится на второе без остатка)
...
if ((number % 7 == 0) && (number % 23 == 0) || (number % 6 == 0))
{
...
else
{
    ...
    Console.WriteLine($"Остаток от деления на 7 равен: {number % 7}");  // знак доллара это интерполяция, раньше подобная запись выглядела вот так => ("Остаток от деления на 7 равен:" + number % 7 + "");
